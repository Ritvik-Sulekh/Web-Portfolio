...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function loadCase(item) {
    ...
    frame.onload = function() {
      /* Check if iframe loaded blank — cross-origin will throw, assume OK */
      try {
        ...
        /* Cross-origin blocked — content exists but we can't read it */
        /* This is actually GOOD — means it loaded */
      }
    };
  ...
  function isBlockedUrl(url) {
    /* Known embeddable domains — everything else gets open button */
    const allowed = [
      'drive.google.com',
      'docs.google.com',
  ...
  function showCaseOpenButton(src) {
    frame.style.display       = 'none';
    placeholder.style.display = 'flex';
    placeholder.innerHTML = `
      <div class="cases__open-wrap">
        <div class="cases__open-icon">🔗</div>
        <p class="cases__open-text">This content cannot be previewed here.</p>
        <a href="${src}" target="_blank" class="cases__open-btn">Open in new tab →</a>
      </div>
    `;
  }
